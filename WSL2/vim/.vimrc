...
function! InsertTemplate()
  let template_dir = expand('/home/sh/.vim/plugged/vim-template/templates')
  let template_files = globpath(template_dir, '**/*', 0, 1)
  ...
  call fzf#run(fzf#wrap({
        ...
        \ 'sink':   function('s:InsertTemplateSink'),
        \ 'options': '--prompt "Select template> "'
        \ }))
endfunction

function! s:InsertTemplateSink(file)
  execute "0r " . fnameescape(a:file)
  redraw!
endfunction
...
" Define a mapping for auto-completing brackets with proper indentatio
inoremap {<CR> {<CR>}<Esc>
